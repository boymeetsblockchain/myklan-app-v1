...
export default function VerifyAccount() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post('https://api.myklan.africa/public/api/settings/verify/account', formData, {
        headers: {
      ...
      Alert.alert("Success", "Your account verification request has been submitted.", [
  ...
  const selectImage = async (setImageCallback: React.Dispatch<React.SetStateAction<string | null>>) => {
  ...
  return (
    <ScrollView style={tw`flex-1 bg-black p-6`}>
      <Text style={tw`text-2xl font-bold text-white mb-4`}>Verify Your Account</Text>
